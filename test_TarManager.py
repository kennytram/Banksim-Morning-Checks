...
def test_tar_creation(tar_manager, monkeypatch):
    ...
    expected_files = [
        ("/path/to/file1.log", "file1.log"),
        ("/path/to/file2.log", "file2.log"),
    ]
...
def test_archive_success(tar_manager, monkeypatch):
    ...
    monkeypatch.setattr(
        tar_manager, "get_file_patterns_paths", lambda d, p: expected_paths
    )
...
def test_archive_failure(tar_manager, monkeypatch):
    ...
    monkeypatch.setattr(
        tar_manager, "get_file_patterns_paths", lambda d, p: Exception("Error")
    )
